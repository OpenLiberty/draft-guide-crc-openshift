...
Remember to use a secure registry for production environments.
...
In this file, the [hotspot=systemImage hotspot=inventoryImage]`image` is the name and tag of the container image that is used. 
The image address is the same OCR address that you logged in to in the previous section.
